...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        $query = "UPDATE profesor SET nombre='${nombre}', apellidoPaterno='${apellidoPaterno}', apellidoMaterno='${apellidoMaterno}', correo='${correo}', clave='${password}', numEmpleado='${numEmpleado}' WHERE correo='${correo}'";
        echo $query;
        $resultado2 = mysqli_query($db, $query) or die(mysqli_error($db));
        if ($resultado) {
            header('location: /maestro/actualizacionPerfil.php?mensaje=1');
        }
    }
}

?>
    <main class="maestro actualizacion">
        <div class="principal">
