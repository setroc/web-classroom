<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errores)) {
            $query = "INSERT INTO profesor (nombre, apellidoPaterno, apellidoMaterno, correo, clave, numEmpleado) VALUES ('$nombre', '$apellidoPaterno', '$apellidoMaterno', '$correo', '$password', '$empleado')";
            echo $query;
            $resultado = mysqli_query($db, $query) or die(mysqli_error($db));
            if ($resultado) {
                header('location: /admin/gestionarMaestro.php?mensaje=1');
            }
        }
    }
